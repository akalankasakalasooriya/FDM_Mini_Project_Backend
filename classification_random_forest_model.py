...
def train_model():
    ...
    with open(os.getcwd()+'/models/classification_models/rf_ml_binarizer.pickle', 'wb') as f:
        pickle.dump(new_md, f)
    ...
    # dump
    with open(os.getcwd()+'/Classif_PlotSum/rf_tf_idf_vectorizer.pickle', 'wb') as f:
        pickle.dump(new_md, f)
    ...
    rfc = RandomForestClassifier(
        n_estimators=20, criterion='gini', random_state=42)
    ...
    print('saving model...')
    # dump the model
    with open(os.getcwd()+'/models/classification_models/rf_ovr_classif.pickle', 'wb') as f:
        pickle.dump(new_md, f)
...
def get_predictions(input):
    # load the model
    load = pickle.dump
    with open(os.getcwd()+'/models/classification_models/rf_ovr_classif.pickle', 'rb') as f:
        rf_ovr_classif_model = pickle.load(f)

    clean_input = data_preprocess(input)

    # load the tfidf vectorizer
    with open(os.getcwd()+'/models/classification_models/rf_tf_idf_vectorizer.pickle', 'rb') as f:
    ...
    input_prediction = rf_ovr_classif_model.predict(input_tfidf)

    with open(os.getcwd()+'/models/classification_models/rf_ml_binarizer.pickle', 'rb') as f:
